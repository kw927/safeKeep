/**
 * CryptoComponent
 * This component is used to display the wallet balance and transactions for the selected chain
 */
'use client';
...
const CryptoComponent = ({ encryptedWallet }: EncryptedWalletProps) => {
    ...
    const chains = [{ name: 'BSC Testnet', chainId: '0x61', symbol: 'BNB', icon: '/images/bsc.png' }];
                    ...
                    <div className='p-6 mx-auto bg-white rounded-xl shadow-md flex flex-col space-y-4 min-w-full'>
                        <div className='text-center'>
                            ...
                            <div className='mt-5'>
                                <p className='text-3xl tracking-tight text-gray-900'>
                                    {formatEther(tokenBalance)} {selectedChain.symbol}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className='flex justify-center items-center gap-4 p-5'>
                            <button
                                ...
                                onClick={() => {
                                    router.push(`/web3/crypto/send?chainId=${selectedChain.chainId}`);
                                }}
                            >
                                Send Token
                            </button>
                            <button
                                ...
                                onClick={() => {
                                    router.push(`/web3/crypto/receive?chainId=${selectedChain.chainId}`);
                                }}
                            >
                                Receive Token
                            </button>
                                        ...
                                        <tbody className='divide-y divide-gray-200'>
                                            ...
                                            {transactions.slice(0, 10).map((tx, index) => (
                                                <tr key={tx.hash}>
                                                    ...
                                                    <td className={classNames('whitespace-nowrap px-3 py-4 text-sm', tx.valueColor)}>
                                                        {tx.direction}
                                                    </td>
                                                    <td className={classNames('whitespace-nowrap px-3 py-4 text-sm', tx.valueColor)}>
                                                        {tx.valuePrefix} {tx.value} {selectedChain.symbol}
                                                    </td>
                                                </tr>
                                            ))}
                                        </tbody>
